...
# Used when the endpoint is a plain OpenAI-compatible one.
...
def _detect_provider(base_url: str) -> str:
    ...
    Anything that is not recognisably Anthropic or Codex is treated as an
    OpenAI-compatible chat-completions endpoint, because that is what almost
    everything is — vLLM, Ollama, LM Studio, OpenRouter, api.openai.com.

    This used to default to `openai-codex`, which sent Codex-shaped
    `/codex/responses` SSE to servers that speak `/chat/completions`. It worked
    only because every call happened to go through a Router that did speak
    Codex; pointed anywhere else it failed, which is the first thing a new
    reader would have tried.
    """
    url = (base_url or "").lower()
    if "anthropic" in url:
        return "anthropic"
    if "chatgpt.com/backend-api" in url or "/codex" in url:
        return "openai-codex"
    return "openai"
...
class LLMClient:
    ...
    def __init__(
        ...
        if (
            not env_provider
            and not provider
            and self.provider in ("openai", "openai-codex")
            and self.api_key
            and not self.api_key.startswith(("sk-", "pk_"))
            and self._is_codex_token(self.api_key)
        ):
            # A ChatGPT subscription token only works against Codex, so it
            # decides the provider even when the URL gave no hint.
        ...
        if not self.default_model:
            if self.provider == "openai-codex":
                ...
                self.default_model = OPENAI_DEFAULT_MODEL
